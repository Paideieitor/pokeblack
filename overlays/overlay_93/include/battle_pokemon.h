...
typedef struct ALIGNED(2) MoveCore {
  u16 move;
  u8 currentPP;
  u8 maxPP;
  u8 PPUpCount;
...
typedef struct MoveSet {
  ...
  MoveCore surface; // Actual move that can be selected at the moment (this can
                    // happen because of Transform and other interactions)
...
typedef struct ALIGNED(4) BattlePokemon {
...
typedef enum BattlePokemonValue {
  ...
  VALUE_LEVEL = 0xF,
  VALUE_ABILITY = 0x10,
  VALUE_EFFECTIVE_ABILITY =
      0x11, // Returns the current active ability, acounting for Gastro Acid
  VALUE_SEX = 0x12,
  VALUE_FORM = 0x13,
